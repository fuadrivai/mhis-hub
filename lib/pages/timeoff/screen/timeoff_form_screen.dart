...
class _TimeoffFormScreenState extends State<TimeoffFormScreen> {
  ...
  Future<void> _submitForm() async {
    ...
    // Prepare form data, skipping fields hidden by show_if conditions.
    ...
    // Trigger form submission via bloc with multipart support
    if (mounted) {
  ...
  Future<void> _handleFormSubmissionState(TimeoffState state) async {
    if (state.isSuccess) {
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (_) => const TimeoffDetailScreen(),
        ),
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocBuilder<TimeoffBloc, TimeoffState>(
        builder: (context, state) {
          // Handle submission state changes
          if (state.isSuccess && state.isSuccess != _lastSuccessState) {
            _lastSuccessState = state.isSuccess;
